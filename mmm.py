...
# def draw_line(cmr: crm.CatmullRomSpline, div):
#     for i, j in itertools.product(range(len(cmr.points)-2-1), range(div)): #Xの10通り、Yの10通りの全組み合わせ
...
def 曲線を再描画する(img, cmr, div):
    ...
    # length = len(cmr.points) - 2 - 1 
    # # div = 80 # 分割数
    # for i, j in itertools.product(range(length), range(div)):
    #     _p = cmr.getValue(i, j/div)
    #     img[int(_p[1]), int(_p[0])] = (255,0,0)
